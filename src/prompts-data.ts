...
export interface ArxivPromptOptions {
...
export function buildArxivPrompt(
  ...
  dateStr: string,
  lang: Lang = "zh",
  options: ArxivPromptOptions = {},
): string {
  const maxPapersPerTopic = options.maxPapersPerTopic ?? 5;
  const includeEmptyTopics = options.includeEmptyTopics ?? false;
...
2. **Research Areas** — Use the exact Markdown hierarchy below. For each topic, keep at most ${maxPapersPerTopic} highly relevant papers. Each item must include abbreviated authors, publication date, one-sentence contribution, and one-sentence relevance.
...
Rules:
- Within each topic, order papers by priority: unseen/new papers first, then stronger keyword relevance, then newer publication date.
...
- Relevance is more important than filling a quota. Omit weak matches.
${includeEmptyTopics ? '- Include every configured topic. Under a topic with no candidate papers, write "No new papers today".' : "- Omit empty topics."}
...
2. **分方向情报** — 必须使用下面固定的 Markdown 阶梯层级。每个方向最多保留 ${maxPapersPerTopic} 篇高相关论文；每篇包含作者缩写、发布日期、一句话核心贡献和一句话与该方向的关联。
   - 主方向：\`## 主方向名\`
   - 子方向：\`### 子方向名\`
   - 文献标题：\`#### [文献标题](ArXiv 链接)\`
...
规则：
- 每个方向内严格按优先级排序：未出现过的新论文最优先，其次按与该方向关键词的相关度由高到低，最后再按发布日期由新到旧。
- 每篇重复文献的条目必须以“🔁 **【过去14天内已出现】**”开头。
- 禁止把主方向和子方向用“/”或其他分隔符写在同一个标题中。
- 主方向、子方向和文献标题前不要添加数字序号。
- 相关性优先，不为凑数量收录弱相关论文。
${includeEmptyTopics ? "- 必须列出每一个已配置方向；某方向没有候选论文时，在该子方向下明确写“今日暂无新论文”。" : "- 没有高相关结果的方向直接隐藏。"}
- 不要创建“其他”或“通用 AI”板块。
